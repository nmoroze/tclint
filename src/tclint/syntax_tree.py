...
class Node:
    ...
    def update_children_links(self):
        """Link the siblings, so it's possible to iterate between them"""
    ...
    def find_by_pos(self, line: int, col: int) -> Node | None:
        """Find the deepest child node in the tree (i.e. most granular match) that matches the given position."""
